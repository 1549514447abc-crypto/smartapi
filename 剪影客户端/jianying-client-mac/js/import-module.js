    ...
    const ImportModule = {
        ...
        render() {
            ...
            importPage.innerHTML = `
                <h2 class="page-title">📥 草稿导入</h2>

                <!-- 未登录/无权限提示 -->
                <div id="accessBlocker" class="callout callout-warning" style="display: none;">
                    <div class="callout-icon">🔒</div>
                    <div>
                        <h4 id="blockerTitle" class="callout-title">请先登录</h4>
                        <p id="blockerMessage" class="callout-message">登录后即可使用草稿导入功能</p>
                    </div>
                </div>

                <div class="bento-grid">
                    <section class="bento-card bento-card--8 bento-card--blue">
                        <h3>🔗 批量草稿链接输入</h3>
                        <div class="form-group">
                            <label for="draftUrl">草稿链接（支持批量输入）：</label>
                            <textarea 
                                id="draftUrl" 
                                class="form-control monospace-input" 
                                rows="6" 
...
• 其他OSS存储链接"
                    ...
                    <section class="bento-card bento-card--4 bento-card--mint">
                        <h3>🎬 一键导入</h3>
                        <p style="color: var(--text-2); font-size: 13px; line-height: 1.6; margin: 0 0 12px 0;">
                            粘贴草稿 JSON 链接后，点击开始导入。进度与日志会在下方实时更新。
                        </p>
                    ...
                    <section class="bento-card bento-card--lilac progress-section-large" id="progressSection">
        ...
        updateAccessBlocker() {
            ...
            // 统一由 updateImportButtonState 决定按钮状态与文案
        ...
        showPathSettingAlert() {
            ...
            if (result) {
                this.goToPathSettings();
            }
        },

        /**
         * 直接跳转到个人中心的路径设置区域（不弹 confirm）
         */
        goToPathSettings() {
            window.JianyingApp.navigation.switchPage('profile');
            setTimeout(() => {
        ...
        updateImportButtonState() {
            ...
            const hasAccess = window.JianyingApp.state.clientAccessAllowed;
            const currentPath =
                window.JianyingApp.modules.pathManager?.getCurrentPath() ||
                window.JianyingApp.state.jianyingPath;
            ...
            if (startBtn && urlInput) {
                ...
                if (!hasAccess) {
                    startBtn.disabled = true;
                    startBtn.textContent = '无使用权限';
                    startBtn.className = 'btn btn-secondary btn-lg';
                    startBtn.dataset.action = 'none';
                    return;
                }

                // 已登录且有权限，但未设置路径：允许点击跳转设置
                if (!hasValidPath) {
                    startBtn.disabled = false;
                    startBtn.textContent = '去设置剪映路径';
                    startBtn.className = 'btn btn-secondary btn-lg';
                    startBtn.dataset.action = 'goToPathSettings';
                    return;
                }
                ...
                startBtn.dataset.action = 'import';
            }
        },
        ...
        async handleStartImport() {
            ...
            // 按钮处于“去设置路径”状态：直接跳转
            const startBtn = document.getElementById('startImportBtn');
            if (startBtn && startBtn.dataset.action === 'goToPathSettings') {
                this.addLogEntry('⚠️ 请先设置剪映草稿路径后再导入', 'warning');
                this.goToPathSettings();
                return;
            }
        ...
        async startSingleImportProcess(draftUrl, jianyingPath, currentIndex = 1, totalCount = 1) {
            ...
            try {
                ...
                // 3. 创建项目目录结构
                // 用草稿名作为文件夹名（和剪映保持一致，避免剪映重命名导致路径断裂）
                const draftName = draftData.name || `草稿_${new Date().toLocaleDateString('zh-CN').replace(/\//g, '-')}`;
                const projectId = this._getSafeFolderName(draftName, jianyingPath);
                    ...
                    for (const resource of resources) {
                        if (resource.downloadUrl) {
                            try {
                                ...
                                // 记录失败信息
                                if (!this._failedResources) this._failedResources = [];
                                this._failedResources.push({ fileName: resource.fileName, reason: error.message });
                                // 继续下载其他文件，不中断整个流程
                            }
                        } else {
                ...
                // 6. 完成 — 汇总下载结果
                const failedList = this._failedResources || [];
                this._failedResources = []; // 重置

                this.addLogEntry(`第 1 个草稿处理完成，保存至: ${projectPath}`, 'success');

                if (failedList.length > 0) {
                    const isExpired = failedList.some(f => f.reason.includes('过期'));
                    this.addLogEntry(`⚠️ 草稿导入完成，但有 ${failedList.length} 个文件下载失败：`, 'warning');
                    failedList.forEach(f => {
                        this.addLogEntry(`  • ${f.fileName}: ${f.reason}`, 'error');
                    });
                    if (isExpired) {
                        this.addLogEntry(`💡 提示：视频下载链接已过期（有效期24小时），请让生成者重新生成草稿链接后再导入`, 'warning');
                    }
                    this.updateProgress('导入完成（部分文件失败）', 100);
                } else {
                    this.addLogEntry(`✅ 下载完成: 剪映草稿已成功下载，所有资源文件完整`, 'success');
         ...
         * 生成安全的文件夹名（处理重名）
         */
        _getSafeFolderName(baseName, parentPath) {
            const fs = window.require('fs');
            const path = window.require('path');

            let folderName = baseName;
            let fullPath = path.join(parentPath, folderName);
            let counter = 1;

            while (fs.existsSync(fullPath)) {
                counter++;
                folderName = `${baseName} (${counter})`;
                fullPath = path.join(parentPath, folderName);
            }
