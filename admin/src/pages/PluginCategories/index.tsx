...
const PluginCategories = () => {
  ...
  const [editingCategory, setEditingCategory] = useState<PluginCategory | null>(null);
  ...
  const handleEdit = (category: PluginCategory) => {
  ...
  const handleDelete = async (id: number) => {
    try {
      await api.delete(`/plugin-categories/${id}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingCategory) {
        await api.put(`/plugin-categories/${editingCategory.id}`, data);
        message.success('更新成功');
      } else {
        await api.post('/plugin-categories', data);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      render: (text: string, record: PluginCategory) => (
        <span>
          {record.icon} {text}
        </span>
      ),
    ...
    },
    {
      title: '操作',
      width: 120,
      render: (_: any, record: PluginCategory) => (
  ...
  return (
    <div>
      <div className="mb-4 flex justify-between items-center">
        <h2 className="text-xl font-semibold">插件分类管理</h2>
        <div className="flex gap-2">
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item
            ...
            extra="英文小写，用下划线分隔，如：ai_image"
          >
            <Input placeholder="如：ai_image" disabled={!!editingCategory} />
          </Form.Item>
